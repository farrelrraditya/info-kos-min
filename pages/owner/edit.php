...
$stmt = $pdo->prepare("SELECT * FROM owners WHERE id_owner = ?");
$stmt->execute([$idOwner]);
$owner = $stmt->fetch();

if (!$owner) {
    setFlash('Data pemilik tidak ditemukan.', 'danger');
    redirect(BASE_URL . '/pages/owner/index.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmtUpdate = $pdo->prepare("UPDATE owners SET owner_name = ?, phone_number = ?, email = ? WHERE id_owner = ?");
            $stmtUpdate->execute([$ownerName, $phoneNumber, $email ?: null, $idOwner]);

            setFlash('Data pemilik "' . $ownerName . '" berhasil diperbarui!', 'success');
            redirect(BASE_URL . '/pages/owner/index.php');
    ...
    <div class="container">
        <div class="row align-items-center">
            <div class="col-md-6 col-12">
                <h2 class="mb-0 text-dark"><i class="bi bi-pencil me-2 text-primary"></i>Edit Pemilik</h2>
            </div>
        ...
        <div class="col-md-8 col-lg-6">
            <div class="card shadow-sm">
                <div class="card-body p-4">
                    <form id="ownerForm" method="POST" action="edit.php?id=<?= $idOwner ?>" novalidate>
                        ...
                        <div class="mb-3">
                            ...
                            <input
                                ...
                                id="owner_name"
                                name="owner_name"
                                value="<?= h($_POST['owner_name'] ?? $owner['owner_name']) ?>"
                                required
                            >
                        ...
                        <div class="mb-3">
                            ...
                            <input
                                ...
                                id="phone_number"
                                name="phone_number"
                                value="<?= h($_POST['phone_number'] ?? $owner['phone_number']) ?>"
                                required
                            >
                        ...
                        <div class="mb-4">
                            ...
                            <input
                                ...
                                id="email"
                                name="email"
                                value="<?= h($_POST['email'] ?? $owner['email']) ?>"
                            >
